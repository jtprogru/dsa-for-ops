...
`mti-dsa` уже вырос из «только структуры данных» в более широкую базу для эксплуатации:
...
1. **Имя репозитория — решено: оставляем `mti-dsa`.** После расширения охвата (Go-трек + Python-фундамент) `dsa` уже не описывает содержание буквально, но имя сохраняется как **историческое и узнаваемое**; фактический охват пояснён в описании, заголовке сайта и README. Переименование остаётся возможным позже, но не является блокером и ломало бы внешние ссылки.
