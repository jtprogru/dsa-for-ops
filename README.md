...
```bash
make py-run              # список доступных Python-лабораторных
make py-run lab01        # запустить конкретную лабораторную
make py-run lab06
```
...
```bash
make py-newlab           # создать следующую по номеру лабу (labNN.py + тест + docs/labNN.md)
make py-newlab lab15     # либо задать номер явно
```

## Линтинг и форматирование

```bash
make lint                # ruff check + ruff format --check (как в CI)
